...
realiza llamadas a Javascript. Tomado de la documentación:
    ...
    build modern user interfaces with the simplicity and power of
    hypertext
...
sección ``head``. No son necesarios procesos previos antes de usarlo. En
otras palabras, no hace falta Javascript en el servidor: Ni
`node <https://nodejs.org/es>`_, ni `npm <https://www.npmjs.com/>`_ o
similares.

- `Documentación oficial de htmx <https://htmx.org/docs/>`_

La filosofía detrás de htmx
------------------------------------------------------------------------

La idea es extender y generalizar las ideas principales de HTML como
*hipertexto*, abriendo más posibilidades directamente en el lenguaje:

- Ahora cualquier elemento, no solo enlaces o formularios, puede
  desencadenar una petición HTTP.

- Ahora cualquier evento, no solo los *clicks* del ratón, puede disparar
  acciones

- Ahora se puede usar cualquier verbo definido en HTTP, no solo ``GET``
  y ``POST``.
...
El núcleo de htmx es un conjunto de atributos que permiten realizar
peticiones AJAX directamente desde el HTML:
...
realizara la petición a la URL correspondiente cuando se dispare el
activador.
...
Cuando el usuario pulse este botón, realiza una petición de tipo
``PUT`` a la dirección ``/messages`` y carga la respuesta dentro del
botón.

Cuales son los activadores de htmx
------------------------------------------------------------------------

Por defecto, las peticiones AJAX se disparan por los eventos “naturales”
de un elemento:

-  Los elementos ``input``, ``textarea`` y ``select`` se activan en un
evento de tipo ``change``.
...
Todos los `eventos
estándar <https://developer.mozilla.org/en-US/docs/Web/API/Element#events>`_
...
``dblclick``       Se realiza un doble *click* con el ratón
...
``scroll``         Se realiza un *scroll* de la vista del documento
``scrollend``      El *scroll* del documento ha terminado
================== ================================================
...
- ``throttle:<time interval>``: Espera durante el tiempo indicado antes
  de realizar la solicitud. Al contrario que con ``delay``, si se
  produce un nuevo evento antes del tiempo límite, el evento se
  descarta, de forma que la petición se realizará al final del contador.

- ``from:<CSS Selector>``: Espeta por un evento de un elemento
  diferente. Se puede usar para implementar atajos de teclado, por
  ejemplo. Hay que hacer notar que el selector CSS no es re-evaluado si
  la página cambia.
